...
def linearize(input_file, output_file):
    """
    Linearize a PDF file for fast web viewing.

    Args:
        input_file (str): Path to the input PDF file.
        output_file (str): Path to the output linearized PDF file.
    ...
    # Construct request body for PDF linearization
    request_body = {
        "documentId": document_id
    }

    # Submit linearization task
    path = "/documents/optimize/pdf-linearize"
    ...
    try:
        ...
        raise Exception(f"Failed to submit linearization task: {e}")
    task_id = response.json().get("taskId")
    if not task_id:
        code = response.json().get("code")
        message = response.json().get("message")
        raise Exception(f"Linearization task submission failed: {code} - {message}")
