...
def manipulate(input_file, output_file,operations):
    """
    Perform various manipulations on a PDF file using Foxit Cloud API.

    Args:
        input_file (str): Path to the input PDF file.
        output_file (str): Path to save the manipulated PDF file.
        operations (list): List of operations to perform (e.g., rotate, reorder pages).
    ...
    # Construct request body for PDF manipulation
    request_body = {
    ...
    path = "/documents/modify/pdf-manipulate"
    ...
    try:
        ...
        raise Exception(f"Failed to submit manipulation task: {e}")
    task_id = response.json().get("taskId")
    if not task_id:
        code = response.json().get("code")
        message = response.json().get("message")
        raise Exception(f"Manipulation task submission failed: {code} - {message}")
